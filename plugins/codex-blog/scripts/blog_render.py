...
Reads a single .md file plus its frontmatter, emits:
  <out-dir>/<slug>.html: self-contained, dark-mode-aware, JSON-LD,
    Open Graph + Twitter Card metadata. Visual metadata is omitted unless
    an explicit --hero filename is supplied.
...
def _validate_hero_filename(name: str) -> str:
    ...
    if relative.is_absolute() or ".." in relative.parts or not relative.parts:
        raise ValueError("hero filename must stay inside the output directory")
    if not all(re.fullmatch(r"[A-Za-z0-9._-]+", part) for part in relative.parts):
        raise ValueError("hero path contains unsupported characters")
    if relative.suffix.lower() not in {".png", ".jpg", ".jpeg", ".webp"}:
        raise ValueError("hero filename must reference a PNG, JPEG, or WebP raster asset")
...
def _markdown_to_html(body: str) -> str:
    ...
    body = DROP_CONTENT_BLOCK.sub("", body)
    try:
...
def _inline(text: str) -> str:
    ...
    # The stdlib fallback cannot safely preserve raw HTML. Strip tags while
    # keeping their visible text; dangerous element bodies were already removed.
    text = RAW_HTML_TAG.sub("", text)
...
def _build_json_ld(fm: dict, word_count: int, og_image_url: str | None = None) -> str:
    data = {
...
def _render_html(md_path: Path, out_dir: Path, hero_filename: str | None = None) -> Path:
    if hero_filename:
        hero_filename = _validate_hero_filename(hero_filename)
        hero_path = (out_dir.resolve() / hero_filename).resolve(strict=False)
        hero_path.relative_to(out_dir.resolve())
        if not hero_path.is_file() or hero_path.is_symlink():
            raise ValueError(f"hero image is missing or unsafe: {hero_filename}")
    ...
    og_image = (
        canonical.rstrip("/") + "/" + hero_filename if canonical and hero_filename else hero_filename
    )
    social_image_tags = ""
    hero_html = ""
    if hero_filename and og_image:
        alt = _safe_attr(fm.get("og_image_alt", title))
        social_image_tags = (
            f'<meta property="og:image" content="{_safe_attr(og_image)}">\n'
            '<meta property="og:image:width" content="1200">\n'
            '<meta property="og:image:height" content="630">\n'
            f'<meta property="og:image:alt" content="{alt}">\n'
            f'<meta name="twitter:image" content="{_safe_attr(og_image)}">'
        )
        hero_html = (
            f'<figure class="hero"><img src="{_safe_attr(hero_filename)}" alt="{alt}" '
            'width="1200" height="630"></figure>'
        )
    ...
    rendered = HTML_TEMPLATE.format(
        ...
        social_image_tags=social_image_tags,
        twitter_card="summary_large_image" if hero_filename else "summary",
...
def _render_pdf(html_path: Path, out_pdf: Path, engine: str) -> bool:
    ...
    if html_path.is_symlink() or not html_path.is_file():
        raise ValueError(f"existing HTML is missing or unsafe: {html_path}")
    if out_pdf.is_symlink():
        raise ValueError(f"refusing to overwrite symlink: {out_pdf}")
    if out_pdf.parent.resolve() != asset_root:
        raise ValueError("PDF output must stay beside the rendered HTML")
    fd, temp_name = tempfile.mkstemp(prefix=f".{out_pdf.name}.", suffix=".tmp", dir=asset_root)
    os.close(fd)
    ...
    try:
            ...
            if sync_playwright is not None:
                try:
                    with sync_playwright() as p:
                        ...
                        page.pdf(path=str(temp_pdf), format="A4", print_background=True)
                        browser.close()
                    if temp_pdf.stat().st_size:
        ...
        if engine in ("auto", "weasyprint"):
            try:
                ...
                HTML(filename=str(html_path), url_fetcher=fetcher).write_pdf(str(temp_pdf))
                if temp_pdf.stat().st_size:
                    os.replace(temp_pdf, out_pdf)
                    return True
            except Exception as e:
...
def main() -> int:
    ...
    parser.add_argument(
        "--hero",
        default=None,
        help="Optional hero image filename relative to out-dir. Omit to render without visuals.",
    )
    parser.add_argument("--pdf-engine", choices=["auto", "playwright", "weasyprint", "none"], default="auto")
    parser.add_argument(
        "--existing-html",
        help="Use this existing HTML for PDF output instead of rendering Markdown again",
    )
    ...
    rendered_html = not bool(args.existing_html)
    if args.existing_html:
        html_path = Path(args.existing_html).absolute()
        if html_path.is_symlink() or not html_path.is_file() or html_path.parent.resolve() != out_dir:
            print(f"ERROR: existing HTML is missing or unsafe: {html_path}", file=sys.stderr)
            return 1
    else:
        try:
    ...
    result = {"html": str(html_path) if rendered_html else None, "pdf": None}
