...
in `skills/blog/SKILL.md`. The orchestrator instructs Codex to invoke
...
def main() -> int:
    ...
    parser.add_argument(
        "--allow-any-basename",
        action="store_true",
        help="Testing only: requires CODEX_BLOG_TEST_ALLOW_ANY_BASENAME=1.",
    )
    ...
    if args.allow_any_basename and os.environ.get("CODEX_BLOG_TEST_ALLOW_ANY_BASENAME") != "1":
        print("Error: --allow-any-basename is only available when CODEX_BLOG_TEST_ALLOW_ANY_BASENAME=1", file=sys.stderr)
        return 2
