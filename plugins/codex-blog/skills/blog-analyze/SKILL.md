...
Read `image_mode` from a nearby `run-manifest.json`. When it is `deferred`
(also assume `deferred` for a fresh article under `.codex-blog/output/`
with no manifest), mark image count, cover/OG image, alt text, image formats,
screenshots, and image-performance checks `N/A`. Do not report their absence as
an issue or reduce the score. Normalize the points earned from applicable
Technical checks back to that category's 15-point maximum so a no-image article
is comparable. Inspect visual quality only when images were enabled or when the
user explicitly requests a visual audit of an existing published page.

The score and rating are advisory. Only an incomplete article, confirmed
factual error, unsafe disclosure, invalid publishable structure, or unresolved
generation placeholder can block the core Markdown under the Blog delivery
contract.
...
- `--cognitive-load`: run `codex-blog run cognitive_load.py` against the post and embed the per-section load heatmap as a sibling `cognitive_load` field. See `skills/blog/references/cognitive-load.md`.
...
Apply the image-mode normalization above before calculating this category. For
deferred mode, evaluate non-image OG/social fields and make every visual
sub-check N/A rather than failed.
...
| Score | Rating | Action |
|-------|--------|--------|
| 90-100 | Exceptional | Publishable; optional polish only |
| 80-89 | Strong | Publishable when core checks are safe |
| 70-79 | Acceptable | Targeted improvements recommended |
| 60-69 | Below Standard | Significant improvement recommended |
| < 60 | Rewrite | Inspect core issues; score alone does not block |
...
The rubric does NOT replace the 100-point score. It runs alongside and surfaces
core corrections versus optional polish; severity alone never overrides the
Blog delivery contract's narrow blocking definition.
...
When `--cognitive-load` is passed, run `codex-blog run cognitive_load.py <file> --format json` and embed the result under a `cognitive_load` field in JSON output, or append a `### Cognitive Load Heatmap` markdown section in markdown output. See `skills/blog/references/cognitive-load.md` for thresholds and interpretation.
