...
For Codex Blog run artifacts, read each nearby `run-manifest.json`. When
`image_mode` is `deferred`, make all cover, OG image, inline-image,
alt-text, screenshot, format, and image-performance checks `N/A`; do not deduct
points or add missing-image actions. Normalize applicable Technical scoring as
defined by `$blog-analyze`. Audit existing published content normally when no
Codex Blog image-mode context exists.
...
```bash
codex-blog run analyze_blog.py <blog-root> --batch --format json
```
...
- Check on-page SEO elements per post:
  ...
  - Image alt text coverage only when real images are present/enabled
...
- Use `blog-data` when available for Core Web Vitals, GSC queries, URL
