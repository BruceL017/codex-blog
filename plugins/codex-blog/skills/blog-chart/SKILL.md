...
description: >
  ...
  source attribution, and transparent backgrounds. Use only when the user
  explicitly requests charts or enables the final visual stage; normal Blog
  writing leaves all charts off.
license: MIT
---

# Blog Chart: Built-In SVG Data Visualization

Generates dark-mode-compatible inline SVG charts for blog posts. A `$blog chart`
invocation is explicit visual intent. `blog-write`, `blog-rewrite`, and cluster
workflows may invoke this Skill only after complete articles and non-image
stages, with visuals enabled. Never insert a chart placeholder when generation
...
```bash
codex-blog run blog-chart/generate_chart_svg.py --input chart.json --output chart.html --json
```
