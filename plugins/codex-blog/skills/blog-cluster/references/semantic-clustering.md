...
Use Codex web search to expand the seed keyword into 30 to 50 candidate phrases:
...
1. Codex web search keyword A. Note the top 10 organic result URLs (domain plus path).
2. Codex web search keyword B. Note the top 10 organic result URLs.
...
Use Codex web search to identify the key entities Google associates with the topic:
...
run `$blog data keywords` with an already configured provider, or optionally use
DataForSEO via an installed `seo-dataforseo` Skill.
