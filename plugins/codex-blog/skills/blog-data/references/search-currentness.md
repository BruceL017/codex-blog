...
2. In a standalone install, use `data/google-updates.json` beside the main
   ...
   `data/google-updates.json` from the trusted installed Codex Blog plugin.
...
- No supported Search Console API endpoint is documented for this dedicated
  view. The blog-data command must report `SKIPPED` or unavailable and direct
...
Do not promise that the current Search Console API or `$blog data gsc`
