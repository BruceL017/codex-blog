#!/usr/bin/env python3
"""
Google API credential management for Codex Blog.
...
CONFIG_PATH = os.path.expanduser(
    os.environ.get(
        "CODEX_BLOG_GOOGLE_CONFIG",
        "~/.config/codex-blog/google-api.json",
    )
)
TOKEN_PATH = os.path.expanduser(
    os.environ.get(
        "CODEX_BLOG_GOOGLE_TOKEN",
        "~/.config/codex-blog/oauth-token.json",
    )
)
...
OAUTH_URLS = {
    "token": ("https://oauth2.googleapis.com/token", "oauth2.googleapis.com"),
    "authorization": (
...
def redact_google_error(value: object, *secret_values: str) -> str:
    """Remove credentials from errors before they enter reports or logs."""
    text = str(value)
    text = re.sub(
...
def _google_oauth_url(value: Optional[str], kind: str) -> str:
    """Return one fixed-host Google HTTPS OAuth endpoint."""
    if kind not in OAUTH_URLS:
        raise ValueError(f"Unknown OAuth endpoint kind: {kind}")
    default, allowed_host = OAUTH_URLS[kind]
    ...
    if (
        ...
        raise ValueError(
            f"OAuth {kind} URL must use HTTPS on {allowed_host}"
        )
    return url
...
def _open_google_oauth(request):
...
def load_config() -> dict:
    ...
    Reads CODEX_BLOG_GOOGLE_CONFIG (or the Codex Blog default config path)
    first. Any missing fields are filled from environment variables.
...
def _refresh_oauth_token(client: dict, token_data: dict) -> Optional[dict]:
    ...
    try:
        token_url = _google_oauth_url(client.get("token_uri"), "token")
        ...
        with _open_google_oauth(req) as resp:
...
def run_oauth_flow(creds_path: str, services: list = None):
    ...
    auth_endpoint = _google_oauth_url(client.get("auth_uri"), "authorization")
    _google_oauth_url(client.get("token_uri"), "token")
...
def print_setup_instructions():
    ...
    print("""
Codex Blog Data - Google API Setup Instructions
================================================
...
6. CREATE CONFIG FILE
   mkdir -p ~/.config/codex-blog
   Save to ~/.config/codex-blog/google-api.json:
...
ENVIRONMENT VARIABLE ALTERNATIVES:
   CODEX_BLOG_GOOGLE_CONFIG     - Override Codex Blog config-file path
   CODEX_BLOG_GOOGLE_TOKEN      - Override Codex Blog OAuth token-file path
...
def main():
    parser = argparse.ArgumentParser(
        description="Google API credential management for Codex Blog"
    )
    parser.add_argument(
        "--check",
        nargs="?",
