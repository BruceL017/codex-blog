    ...
    try:
        ...
        if resp.status_code == 429:
            ...
            result["error"] = (
                ...
                + redact_google_error(resp.text, api_key or "")
            )
        else:
            result["error"] = (
                f"PSI API error {resp.status_code}: "
                + redact_google_error(e, api_key or "")
            )
        return result
    except requests.exceptions.RequestException as e:
        result["error"] = "Request failed: " + redact_google_error(e, api_key or "")
        return result
...
def query_crux(
    ...
    try:
        ...
        result["error"] = "CrUX API request failed: " + redact_google_error(
            e, api_key
        )
        return result
