...
Usage:
    python youtube_search.py search "codex seo"
...
try:
    from google_auth import get_api_key, redact_google_error
except ImportError:
    import os
...
def _build_youtube_service(api_key: Optional[str] = None):
    ...
    try:
        ...
        print(
            "Error building YouTube service: " + redact_google_error(e, key),
            file=sys.stderr,
        )
        return None
...
def search_videos(
    ...
    try:
        ...
        if "403" in error_str:
            ...
            result["error"] = "YouTube API error: " + redact_google_error(
                e, api_key or ""
            )
