...
```bash
codex-blog run content_decay.py current.json previous.json
```

Useful options:

```bash
codex-blog run content_decay.py current.json previous.json --threshold 0.30
codex-blog run content_decay.py current.json previous.json --metric impressions
codex-blog run content_decay.py current.json previous.json --format markdown --output decay-report.md
```

The script accepts JSON lists of page rows with `page` or `url`, `clicks`, and
`impressions`. It also accepts the object shape returned by `blog-data` when
the rows are under a top-level `rows` key.

## Optional Live GSC Export

For live data, use `$blog data gsc` or the underlying `blog-data` command
to create each period export, then run the offline analyzer:

```bash
codex-blog run blog-data/run.py gsc_query --property sc-domain:example.com --dimensions page --start-date YYYY-MM-DD --end-date YYYY-MM-DD --json > gsc-current.json
codex-blog run blog-data/run.py gsc_query --property sc-domain:example.com --dimensions page --start-date YYYY-MM-DD --end-date YYYY-MM-DD --json > gsc-previous.json
codex-blog run content_decay.py gsc-current.json gsc-previous.json --format markdown
```
...
Use `blog-data` to collect Search Console exports when live credentials are
