...
Verify statistics, claims, and source attributions in blog posts. Codex-native
...
2. Fetch the source page via Codex web open/fetch only after those checks pass.
...
codex-blog applies FLOW's evidence discipline through claim-appropriate
...
- **Paywalled content**: Codex web open/fetch cannot access content behind login walls. These
  score as WEAK (0.5) with a note about paywall detection.
- **Dynamic pages**: JavaScript-rendered content may not be available via Codex web open/fetch.
  If the page returns minimal content, note this in the status.
- **PDF sources**: Codex web open/fetch may not extract PDF text reliably. Flag PDF URLs for
  manual verification.
