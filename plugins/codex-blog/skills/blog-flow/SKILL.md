...
workflows. Codex Blog integrates the FLOW prompt library so writers can turn
...
3. State that local-SEO prompts are excluded by design; point users to
   optional Codex SEO `$seo-flow` local analysis if they need them.

### On `$blog flow sync`
1. Run: `codex-blog run blog-flow/sync_flow.py`.
...
- `codex-blog run blog-flow/sync_flow.py`. Sync the latest version of every blog-relevant
  stage to disk and refresh the lockfile.
- `codex-blog run blog-flow/sync_flow.py --dry-run`. Report planned changes without
  writing.
- `codex-blog run blog-flow/sync_flow.py --ref <sha>`. Pin fetches to a specific FLOW
