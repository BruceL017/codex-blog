...
### Optional: Search Performance Context (blog-data)

If blog-data credentials include Tier 1 (GSC) and the post has a published URL:

1. Query GSC by page and query dimensions, then filter rows to the URL:
   `codex-blog run blog-data/run.py gsc_query --property <property> --dimensions query,page --json`
...
3. Check indexation: `codex-blog run blog-data/run.py gsc_inspect <url> --json`
...
```bash
codex-blog run ai_citation_score.py <file> --format markdown
```
