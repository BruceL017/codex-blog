...
When a nearby Blog manifest has `image_mode=deferred`, mark image and
chart parity checks `N/A`; do not count their absence as translation drift or a
quality issue. If visuals were explicitly enabled, compare only real assets and
translated visible labels.
...
> Design credit: Chris Mueller's `claude-blog-multilingual` (Pro Hub Challenge,
> March 2026). Codex Blog ports only the clean-room integration published in
> MIT-licensed claude-blog 2.1.1; see `THIRD_PARTY.md` for the license boundary.
...
If `seo-hreflang` from codex-seo is installed, suggest running it for
deeper validation.
