...
## Content-first and batch image policy

Complete the source Markdown and every possible translated/localized Markdown
article before optional hreflang, sitemap, schema, rendering, or visual work.
Each downstream stage gets an initial attempt and one retry, then is skipped
with a batch warning; it cannot withhold completed articles.

Default `image_mode=deferred`: do not generate/search images, charts, maps, or
screenshots, add placeholders, or penalize missing visuals. After every article
and all non-image stages finish, ask the standard image question once for the
whole multilingual batch. No answer means no images. If images were explicitly
selected in the initial request, generate them last without asking again and
preserve the source/translation prose.
...
It stops at those local artifacts and never publishes or uploads posts to a CMS.

> Design credit: Chris Mueller's `claude-blog-multilingual` (AI Marketing Hub
> Pro Hub Challenge, March 2026). Codex Blog ports only AgriciDaniel's
> clean-room integration published in MIT-licensed claude-blog 2.1.1; see
> `THIRD_PARTY.md` for the source and license boundary. That integration removed
> the original installer and credential handling, and uses the shared
> cultural-adaptation reference under `blog-translate/references/`.
...
|-----------|--------|----------|
| `blog-write` | codex-blog (this plugin) | Yes |
| `blog-translate` | codex-blog (this plugin) | Yes |
| `blog-localize` | codex-blog (this plugin) | Yes (when `--localize` is on, default) |
| `seo-hreflang` | codex-seo (sibling plugin) | No, falls back to a self-contained generator |
...
Generate three artifacts plus localized schema with the self-contained
generator below. If `seo-hreflang` from Codex SEO is installed **and the user
explicitly requests that external validation**, it may validate the result;
absence or failure never changes the Blog output.
...
- If Codex SEO is installed and explicitly requested, run `$seo-hreflang multilingual/` for
  deeper validation.
```
...
| For deeper hreflang validation | `$seo-hreflang <directory>` (Codex SEO, optional) |

## Error Handling

| Scenario | Action |
|----------|--------|
| `blog-write` missing | Error: "This skill requires `blog-write`. Reinstall codex-blog." |
