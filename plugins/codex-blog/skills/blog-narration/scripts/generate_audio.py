#!/usr/bin/env python3
"""
Optional Blog Narration audio generator - Gemini TTS compatibility helper
...
SECRET_ENV_VARS = ("GOOGLE_AI_API_KEY", "GEMINI_API_KEY")


def _redact_sensitive_text(value: object) -> str:
    """Remove credentials that may be reflected by SDK exceptions."""
    text = str(value)
    ...
    text = re.sub(
        r"(?i)\b(GOOGLE_AI_API_KEY|GEMINI_API_KEY)(\s*[=:]\s*)[^\s,;]+",
        r"\1\2[REDACTED]",
        text,
    )
...
def output_result(data: dict, as_json: bool):
    ...
    if data.get("status") == "error":
        data = {**data, "error": _redact_sensitive_text(data.get("error", "Unknown error"))}
    if as_json:
