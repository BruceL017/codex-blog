...
## Delivery and image-mode policy

This check is a downstream enhancer. It receives one initial attempt and one
retry; if it still cannot run, record the limitation and preserve the complete
Markdown. Its aggregate score never withholds the article.

Read `image_mode` from `run-manifest.json`. For `deferred`, mark
`og:image`, Twitter image/card variants that require an image, cover images,
inline-image alt text, dimensions, and image performance `N/A`. Their absence is
neither a failure nor a recommendation to generate images. Validate them only
after an explicitly enabled image stage has produced real assets.
...
Skip image-specific rows as `N/A` under deferred mode; continue to validate
the text and URL fields.
...
Skip image-specific rows as `N/A` under deferred mode. A text-only card is
acceptable for this pre-image article state.
...
### Optional: Live Performance Check (blog-data)

If the post has a published URL and blog-data credentials are available:

1. Check credentials: `codex-blog run blog-data/run.py google_auth --check --json`
2. If Tier 0+, run PageSpeed: `codex-blog run blog-data/run.py pagespeed_check <url> --json`
3. Append to report:
