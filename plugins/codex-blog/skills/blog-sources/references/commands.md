...
CLI documentation for the optional Blog Sources NotebookLM compatibility
scripts. Codex Blog does not require or invoke this connector by default.

Connector state defaults to
`~/.config/codex-blog/sources/notebooklm/`. Set
`CODEX_BLOG_SOURCES_DATA_DIR` to an explicit directory to override it. Browser
credentials and notebook metadata are never stored inside the installed Skill.
...
```bash
codex-blog run blog-sources/run.py <script_name>.py [arguments]
```
...
codex-blog run blog-sources/run.py ask_question.py --question "Your question"

# Query specific notebook by ID
codex-blog run blog-sources/run.py ask_question.py --question "..." --notebook-id notebook-id

# Query by URL directly
codex-blog run blog-sources/run.py ask_question.py --question "..." --notebook-url "https://..."

# JSON output for structured responses
codex-blog run blog-sources/run.py ask_question.py --question "..." --json

# Show browser for debugging
codex-blog run blog-sources/run.py ask_question.py --question "..." --show-browser
```
...
codex-blog run blog-sources/run.py notebook_manager.py add \
...
# List all notebooks
codex-blog run blog-sources/run.py notebook_manager.py list

# Search by keyword (searches name, description, topics)
codex-blog run blog-sources/run.py notebook_manager.py search --query "keyword"

# Set active notebook (default for queries without --notebook-id)
codex-blog run blog-sources/run.py notebook_manager.py activate --id notebook-id

# Remove notebook from library
codex-blog run blog-sources/run.py notebook_manager.py remove --id notebook-id

# Show library statistics
codex-blog run blog-sources/run.py notebook_manager.py stats
```
...
codex-blog run blog-sources/run.py ask_question.py \
...
```bash
codex-blog run blog-sources/run.py auth_manager.py setup    # Initial setup (browser visible)
codex-blog run blog-sources/run.py auth_manager.py status   # Check authentication
codex-blog run blog-sources/run.py auth_manager.py reauth   # Re-authenticate
codex-blog run blog-sources/run.py auth_manager.py clear    # Clear all auth data
```
...
```bash
codex-blog run blog-sources/run.py cleanup_manager.py                    # Preview (dry run)
codex-blog run blog-sources/run.py cleanup_manager.py --confirm          # Execute cleanup
codex-blog run blog-sources/run.py cleanup_manager.py --confirm --preserve-library  # Keep notebooks
codex-blog run blog-sources/run.py cleanup_manager.py --confirm --force  # Skip confirmation
```
...
codex-blog run blog-sources/run.py notebook_manager.py search --query "marketing"
...
codex-blog run blog-sources/run.py ask_question.py --question "..." --notebook-id source-a
codex-blog run blog-sources/run.py ask_question.py --question "..." --notebook-id source-b
...
for question in "Q1" "Q2" "Q3"; do
  codex-blog run blog-sources/run.py ask_question.py --question "$question" --json
