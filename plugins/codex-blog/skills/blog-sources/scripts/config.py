"""
Configuration for the optional Blog Sources NotebookLM connector
...
# Paths. Connector state is user data, not plugin code; installed plugin
# directories may be read-only and must never contain browser credentials.
SKILL_DIR = Path(__file__).parent.parent
DATA_DIR = Path(
    os.environ.get(
        "CODEX_BLOG_SOURCES_DATA_DIR",
        "~/.config/codex-blog/sources/notebooklm",
    )
).expanduser()
