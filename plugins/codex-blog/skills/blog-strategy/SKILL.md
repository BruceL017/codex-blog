...
Strategy planning should consider the FLOW 5-surface model (owned site, SERP plus AI Overviews, AI assistant citations, local pack, communities and video). Local-pack work can be delegated to the optional Codex SEO plugin when installed; the independent Blog workflow covers the remaining surfaces and continues without that plugin. Full mapping in `skills/blog/references/flow-alignment.md`.
...
1. Codex web search for competitor blog URLs
...
Map competitor visibility across AI platforms. Codex web search cannot inspect
...
Use the score as an editorial diagnostic, not a publication gate. Review the
specific findings and revise confirmed safety, accuracy, intent, or completeness
problems; a score below 80 alone cannot withhold a complete article.
