...
Visuals are opt-in. With `image_mode: deferred`, do not generate,
source, request, score, or place images, charts, screenshots, video embeds, or
visual placeholders. Missing visuals are not a quality defect. If the user
enables visuals, add only assets that clarify, prove, or summarize adjacent
content and stay within the site's page-weight budget.

| Parameter | Guidance when visuals are enabled |
|-----------|------------------------------------|
| Image/visual frequency | Intent-based; no fixed minimum |
| Bold/emphasis | Use sparingly for genuine scan points |
| Bold % of total text | Keep below 10% as a style heuristic |
...
### Visual rhythm when enabled

Do not use a visual-per-word quota. Place a useful asset near the passage it
supports and avoid repetitive decorative media. For real enabled images, set
explicit dimensions; do not lazy-load the LCP image; lazy-load below-fold
images where appropriate. These implementation details never apply when
visuals are deferred or disabled.
...
Follow the user's explicit punctuation preference, then loaded brand/voice
guidance. With neither present, avoid U+2014 and use commas, hyphens, colons, or
periods. Under that default, split sentences when a long dash joins two
independent clauses.
