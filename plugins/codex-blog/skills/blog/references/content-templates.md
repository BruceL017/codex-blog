...
Template markers are authoring instructions, never publishable copy. With
`image_mode: deferred`, omit every `[IMAGE]` and `[VISUAL]` marker and
all corresponding fields; do not replace them with placeholders. If visuals are
explicitly enabled, the article still finishes first and the last-stage image
workflow replaces applicable markers with validated real assets.
...
| Complete optimization | Relevant scoring elements included | Answer-first treatment, warranted Q&A, citations, and optional visuals |
...
| `[VISUAL: chart-type]` | Optional visual planning note; omit unless visuals are enabled | `[VISUAL: grouped-bar]` for verified before/after data |
| `[IMAGE]` | Optional image planning note; omit unless visuals are enabled | Place a real relevant asset near the supported passage |
...
| Schema, performance, and enabled visual metadata | Technical Elements | 15 pts; visual criteria are N/A when deferred |
...
`~/.codex/skills/blog/templates/` or in this repo at `skills/blog/templates/`.
...
3. Add markers where they help:
   ...
   - `[VISUAL]` or `[IMAGE]` only where an enabled visual would add meaning
