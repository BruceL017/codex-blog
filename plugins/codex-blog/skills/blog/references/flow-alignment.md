# FLOW Alignment in codex-blog

This reference explains how codex-blog adopts the FLOW framework. Load it on demand when a user asks about FLOW, evidence standards, surface coverage, or how the blog skills route to FLOW stages.

## 1. What FLOW is and why codex-blog adopted it

FLOW is an evidence-led 2026-search operating model published at `github.com/AgriciDaniel/flow` (CC BY 4.0 prompt content, MIT code), authored by Daniel Agrici. It treats modern discovery as a multi-surface system rather than a single SERP, and it encourages traceable support for material public claims. codex-blog v2.1.0 includes FLOW through the `blog-flow` sub-skill (originally integrated in v1.7.0) plus this alignment doc, which informs how every other blog skill in the suite plans, writes, optimizes, and audits content. FLOW provides the principles and prompts; codex-blog provides the production tooling that applies them at scale.

## 2. Evidence records in codex-blog
...
| Surface | What it is | Where codex-blog handles it |
|---|---|---|
...
| 4. Local pack | Map listings, Google Business Profile | Out of scope for blog content. Use optional Codex SEO `$seo-local` or `$seo-maps`. |
...
codex-blog operationalizes surfaces 1, 2, 3, and 5 directly. Surface 4 can be delegated to the optional Codex SEO plugin when it is installed; the Blog core does not require it. When a blog post targets a query that also surfaces in a community (Reddit thread, YouTube comment), the writer should consider dual-surface thinking: optimize the post for extraction AND consider whether the same answer should also live in the community where the query is asked.

## 4. FLOW stages mapped to existing codex-blog skills

| FLOW stage | codex-blog skills that consume this | When to invoke `$blog flow <stage>` directly |
|---|---|---|
...
The Leverage stage (off-site authority, 1 prompt) and Local stage (11 GBP/citation prompts) are intentionally not exposed as top-level codex-blog commands. Leverage is reachable through `$blog flow prompts`. Local can be delegated to the optional Codex SEO plugin; without it, Blog reports that surface as out of scope and continues.

## 5. What codex-blog adds beyond FLOW

- Optional visual media pipeline: provider-neutral image generation (`blog-image`), inline SVG charts (`blog-chart`), and licensed stock sources. It runs only after explicit visual intent; FLOW does not prescribe asset generation.
...
- Topic-cluster execution engine (`blog-cluster`). FLOW prescribes content planning; codex-blog also executes clusters end-to-end with auto-interlinks.
...
- Audio narration (`blog-narration`). Not in FLOW scope.
- Google API integration (`blog-data`: PSI, CrUX, GSC, GA4, NLP, YouTube, Keywords). FLOW references measurement; codex-blog provides API tooling.
- Source-grounded research via NotebookLM (`blog-sources`). Adjacent to FLOW's evidence discipline; provides the research side.
...
- This doc (`flow-alignment.md`): the orchestrator and any sub-skill loads this when the user asks "what does codex-blog do for AI citations / evidence / surfaces", to give a codex-blog-specific answer with skill names and routing.
...
Last updated 2026-07-23 for codex-blog v2.1.0.
