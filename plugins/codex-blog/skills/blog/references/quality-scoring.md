...
When a Codex Blog manifest sets `image_mode` to `deferred`, every
image, cover, Open Graph image, screenshot, chart, alt-text, image-format, and
image-performance item is `N/A`. Missing visuals generate no issue and lose no
points. Score the applicable Technical items, then normalize that subtotal to
15 points. Inspect visuals only when they were explicitly enabled or the user
requests a visual audit of an existing published page.

The 100-point score is an editorial heuristic, never a delivery gate. The Blog
delivery contract blocks only incomplete or factually unsafe core Markdown.
...
- No non-image OG/social metadata (image fields are N/A when visuals are deferred)
- Paragraphs whose pacing or mixed ideas reduce comprehension
- Passive voice > 15%

### Medium Priority
- Fewer than 2 charts, only when visuals were explicitly enabled and charts were required by the user
- Missing requested images, only when visuals were explicitly enabled
...
- Requested images not in an appropriate optimized format
- `loading="lazy"` on an enabled LCP image
- Average sentence length > 22 words
- Transition words < 15% or > 35%

### Low Priority
- Localized paragraph pacing that mildly slows comprehension
- Missing requested chart type diversity
- Enabled images without alt text
