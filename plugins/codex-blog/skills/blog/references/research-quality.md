...
codex-blog already enforces source-tier hygiene (Tier 1 to 3), source fidelity,
...
| Format compliance | 10 | Research synthesis uses citations inline as `[name](url)` per `synthesis-contract.md`. Avoid a duplicate trailing Sources block when active instructions permit. Publishable drafts may include retrieval notes when they identify a changeable or undated source or change interpretation. Use the user's punctuation preference, then brand/voice guidance, then the contract default. No invented titles or raw-cluster dumps. |
