...
Load and apply this reference only after the user explicitly enables visuals.
The normal write workflow keeps `image_mode: deferred`, produces no image,
chart, screenshot, video, provider probe, visual metadata, or placeholder, and
asks once after all non-image work is complete. Missing visuals never reduce the
article score. When enabled, create real assets last and remove every planning
marker before updating publishable Markdown.

## Cover Images & OG Images

When cover generation is explicitly enabled, a cover can support social sharing
and blog listings. Reuse it for Open Graph unless the user requests a separate
asset.
...
1. Run `$blog image cover` only after the article and non-image artifacts exist.
2. Use the provider-neutral prompt brief in `blog-image`.
3. Follow the provider order: Codex-native, configured OpenAI-compatible or
   Gemini-compatible HTTP API with custom base URL support, then configured MCP.
4. Post-process to the requested social dimensions and WebP/AVIF when the
   available provider/runtime supports it.
...
- Add these fields only after the referenced real files validate.
...
- **Hotlinking**: Not allowed by codex-blog policy. Use the source URL only to download a local asset, then serve the local file.
...
- **Hotlinking**: Not allowed by codex-blog policy. Use official API metadata to select an image, then download/cache it locally and keep source metadata.
...
- **Finding**: Codex web search `site:pexels.com [topic keywords]`
...
domains are not required for codex-blog generated assets:
```typescript
...
When enabled assets have multiple formats, use the `<picture>` element for
format negotiation:
