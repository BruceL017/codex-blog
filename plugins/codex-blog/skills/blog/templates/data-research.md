...
Optional visual guidance, only when images are explicitly enabled (horizontal-bar chart showing the top 5-7 findings ranked by magnitude or importance): 1. **[Finding as a stat]:** [One sentence of context]. ([N]% of [sample])
...
Optional visual guidance, only when images are explicitly enabled (chart appropriate to the finding - vary chart types across findings): Suggested chart types per finding:
...
Internal-link opportunity, only when a verified destination URL is supplied (link to content that helps the reader act on this finding):

```

**Rules for finding sections:**
- Always open with the data point - never bury the lead
- When visuals are explicitly enabled, visualize only findings that benefit
  from it and vary chart types where that improves comprehension
...
Internal-link opportunity, only when a verified destination URL is supplied (link to tutorial or guide content that helps implement these recommendations):

Optional visual guidance, only when images are explicitly enabled (summary infographic or decision matrix if applicable):
...
- [ ] Visual markers are omitted when deferred; enabled visuals are useful,
  verified, and varied where appropriate
