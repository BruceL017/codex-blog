...
Internal-link opportunity, only when a verified destination URL is supplied: Link to a related foundational concept post (e.g., "If you're new to [topic], start with our [Beginner's Guide to X]").
...
Optional image guidance, only when images are explicitly enabled: Screenshot or diagram showing the tools/environment the reader should have ready before starting.
...
Optional image guidance, only when images are explicitly enabled: Screenshot showing the expected state after completing this step.
...
Optional image guidance, only when images are explicitly enabled: Screenshot of expected state after this step.
...
Optional visual guidance, only when images are explicitly enabled (flowchart): If the process branches or has decision points at this stage, include a flowchart showing the paths.
...
Optional image guidance, only when images are explicitly enabled: Screenshot showing the final completed state.

Optional visual guidance, only when images are explicitly enabled (before-after): Side-by-side comparison showing before (Step 1) and after (Step 6) state.
...
Optional image guidance, only when images are explicitly enabled: Screenshot of the final successful result.

Optional visual guidance, only when images are explicitly enabled (metrics-dashboard): If applicable, show a performance or status dashboard screenshot.

Internal-link opportunity, only when a verified destination URL is supplied: Link to an advanced guide or next-step post: "Now that you've set up [X], learn how to [optimize/scale/extend it]."
...
Internal-link opportunity, only when a verified destination URL is supplied: Link to 2-3 related posts for continued reading.
...
- [ ] When images are explicitly enabled, visuals appear only at steps where
      they materially improve verification; deferred mode requires none
