...
Internal-link opportunity, only when a verified destination URL is supplied: Link to a foundational guide: "New to [category]? Read our [What Is X and Why Does It Matter] guide first."
...
Optional image guidance, only when images are explicitly enabled: Product screenshot, logo, or UI screenshot showing the key feature in action.
...
Optional image guidance, only when images are explicitly enabled: Product screenshot or key feature visual.
...
Optional visual guidance, only when images are explicitly enabled (comparison-table): **Format:** A markdown table comparing all items across 5-7 key dimensions.
...
Internal-link opportunity, only when a verified destination URL is supplied: Link to 2-3 related posts: individual reviews, comparison posts, or how-to guides for the top picks.
...
- [ ] When images are explicitly enabled, each included visual materially helps
      compare or understand its item; deferred mode requires none
