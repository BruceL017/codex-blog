...
- `image_mode=deferred` is the default. Do not generate, request, plan, or insert images or visual instructions unless the user explicitly enables images.
- Every square-bracket drafting token below is an instruction or variable, never literal article text. Replace it with final copy/data or delete it; real Markdown links and citations are unaffected. Never emit image, visual, internal-link, TODO, or other drafting markers. With images deferred, omit visual instructions completely.
- Add an internal link only when a verified destination URL is supplied. If a relevant destination is unknown, omit the link from the article and record the opportunity in the delivery report.
- When images are explicitly enabled, replace optional visual guidance with real accessible assets; never publish the guidance itself.
...
Optional image guidance, only when images are explicitly enabled: Hero image that visually represents the tension between convention and your alternative. Consider a split-screen concept, a "myth vs. reality" graphic, or a bold typographic treatment of the contrarian claim.
...
Internal-link opportunity, only when a verified destination URL is supplied: If you have previously written content aligned with the conventional view (or a neutral explainer), link to it here for context.
...
Optional image guidance, only when images are explicitly enabled: Include a visual that illustrates one of the problems - a failed example, a misleading chart corrected, or a before/after comparison.
...
Optional visual guidance, only when images are explicitly enabled (chart-type): This is the most important visual in the piece. Include a chart, graph, or diagram that makes your case visually compelling. Options:
...
Internal-link opportunity, only when a verified destination URL is supplied: Link to any supporting data, case studies, or research that deepens the reader's understanding of your alternative view.
...
Internal-link opportunity, only when a verified destination URL is supplied: Link to how-to guides or tutorials that walk the reader through implementing your approach.
...
Internal-link opportunity, only when a verified destination URL is supplied: Link to tactical how-to articles, tools, or templates that support implementation.
...
Internal-link opportunity, only when a verified destination URL is supplied: Link FAQ answers to supporting evidence, case studies, or expanded discussions of these objections.
...
Internal-link opportunity, only when a verified destination URL is supplied: Link to your most relevant related content - the piece you want this audience to read next.
...
- [ ] No visual drafting guidance appears when images are deferred; enabled visuals are real, accessible assets.
- [ ] All [INFO-GAIN] markers replaced with original data, experience, or insight
- [ ] Every internal link uses a verified supplied URL; unknown opportunities appear only in the delivery report.
