...
**Examples:**
- "Codex Tutorial: Building a Blog Automation Pipeline in 2026"
...
Optional image guidance, only when images are explicitly enabled (screenshot or demo of the completed project):
...
Optional visual guidance, only when images are explicitly enabled (simple-diagram showing components/data flow):

```

Internal-link opportunity, only when a verified destination URL is supplied (link to any prerequisite tutorials or foundational concepts):
...
Optional image guidance, only when images are explicitly enabled (screenshot of setup completion / expected terminal output):
...
Optional image guidance, only when images are explicitly enabled (screenshot showing the result of this step):
...
Internal-link opportunity, only when a verified destination URL is supplied (link to deeper explanation of key concepts used in this step):
...
Optional image guidance, only when images are explicitly enabled (screenshot of passing tests):
...
Optional visual guidance, only when images are explicitly enabled (flowchart of the verification process if complex):
...
**Official resources:**
- [Official documentation title](verified official URL)
- [Repository or examples title](verified official URL)
```

Add a **Related tutorials** list only when exact internal destination URLs were
supplied and verified. Otherwise omit it and put those opportunities in the
delivery report.
...
Internal-link opportunity, only when a verified destination URL is supplied (link to content that covers this question in depth):
...
- [ ] Image markers are omitted when deferred; any enabled screenshots support
  key moments
...
- [ ] Visual markers are omitted when deferred; any enabled architecture or
  flow diagram improves comprehension
