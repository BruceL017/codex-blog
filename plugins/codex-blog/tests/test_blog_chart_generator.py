...
def test_atomic_writer_has_windows_compatible_fallback(tmp_path, monkeypatch):
    generator = load_generator()
    destination = tmp_path / "chart.html"
    monkeypatch.setattr(generator, "_supports_dir_fd", lambda: False)

    generator.write_text_atomic_no_follow(destination, "safe chart")

    assert destination.read_text(encoding="utf-8") == "safe chart"
