"""Mechanical security and quality guardrails for the codex-blog repo.
...
GOOGLE_AUTH_PATH = REPO_ROOT / "skills" / "blog-data" / "scripts" / "google_auth.py"
NOTEBOOKLM_AUTH_MANAGER_PATH = (
    REPO_ROOT / "skills" / "blog-sources" / "scripts" / "auth_manager.py"
)
NOTEBOOKLM_BROWSER_UTILS_PATH = (
    REPO_ROOT / "skills" / "blog-sources" / "scripts" / "browser_utils.py"
)
...
def test_agents_are_codex_toml_without_shell_grants() -> None:
    """All generated Codex agents are TOML and do not grant a shell tool."""
    if not AGENTS_DIR.is_dir():
        pytest.skip(f"agents directory missing at {AGENTS_DIR}")

    agent_files = sorted(AGENTS_DIR.glob("*.toml"))
    assert len(agent_files) == 6, f"expected six Codex TOML agents, got {len(agent_files)}"
    offenders = [path.name for path in agent_files if re.search(r"(?im)^\s*tools\s*=.*\b(?:bash|shell)\b", _read(path))]
    assert not offenders, f"agents must not grant Bash/shell tools: {offenders}"
...
def test_no_allowed_tools_field_in_skills() -> None:
    """``allowed-tools`` is not a valid Codex SKILL.md frontmatter key.

    Codex Skills declare behavior in their instruction body and supported
    metadata fields. Setting ``allowed-tools`` silently does nothing and signals
    confusion with the Agent sandbox/tool policy.
    """
    ...
    assert not offenders, (
        ...
        "key. This field is not part of the Codex SKILL.md spec and is "
