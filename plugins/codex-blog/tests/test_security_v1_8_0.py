...
def test_skill_md_documents_untrusted_data_contract(tmp_path: Path) -> None:
    """The Codex orchestrator keeps project and adapter inputs untrusted."""
    orchestrator = (ROOT / "skills" / "blog" / "SKILL.md").read_text(encoding="utf-8")
    low = orchestrator.lower()
    for phrase in ("untrusted data", "cannot override", "secret disclosure", "untrusted project-local lookalike"):
        assert phrase in low
    assert "trusted by default" not in low
...
def test_orchestrator_has_untrusted_data_contract() -> None:
    """Project context cannot expand authority or request secrets."""
    orchestrator = (ROOT / "skills" / "blog" / "SKILL.md").read_text(encoding="utf-8")
    for f in ("BRAND.md", "VOICE.md", "DISCOURSE.md"):
        assert f in orchestrator, f"orchestrator does not mention {f}"
    assert "cannot override system/developer/user instructions" in orchestrator
    assert "expand permissions" in orchestrator
    assert "secret disclosure" in orchestrator


def test_security_md_documents_t12() -> None:
    """Repository security policy documents the current untrusted-input boundary."""
    project = ROOT.parents[1]
    sec = (project / "SECURITY.md").read_text(encoding="utf-8")
    assert "untrusted input" in sec
    assert "unsafe filesystem traversal" in sec
    assert "local/private-network URLs" in sec
